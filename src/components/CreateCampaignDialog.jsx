    ...
    const handleSubmit = () => {
        ...
        if (generatedData) {
            ...
            if (generatedData.adGroups && Array.isArray(generatedData.adGroups)) {
                const maxAdGroupId = adGroups.reduce((max, ag) => Math.max(max, ag.id || 0), 0);
                generatedData.adGroups.forEach((ag, index) => {
                    const adGroupId = maxAdGroupId + index + 1;
                    adGroupMap[ag.name] = adGroupId;
                    addAdGroup({
                        id: adGroupId,
                        'Ad Group': ag.name,
                        Campaign: campaignName.trim(),
                        'Max CPC': ag.maxCpc || 0.5,
            ...
            if (generatedData.keywords && Array.isArray(generatedData.keywords)) {
                const maxKeywordId = keywords.reduce((max, k) => Math.max(max, k.id || 0), 0);
                generatedData.keywords.forEach((kw, index) => {
                    addKeyword({
                        id: maxKeywordId + index + 1,
                        Keyword: kw.keyword,
                        'Ad Group': kw.adGroup,
                        Campaign: campaignName.trim(),
                        'Criterion Type': kw.matchType || 'Broad',
                        Status: 'Enabled',
                    });
                });
            }
            ...
            if (generatedData.ads && Array.isArray(generatedData.ads)) {
                const maxAdId = ads.reduce((max, a) => Math.max(max, a.id || 0), 0);
                generatedData.ads.forEach((ad, index) => {
                    addAd({
                        id: maxAdId + index + 1,
                        'Ad Group': ad.adGroup,
                        Campaign: campaignName.trim(),
                        ...
                        'Final URL': selectedSite?.url || 'https://example.com',
                        Status: 'Enabled',
                    });
                });
            }
                ...
                return (
                    <Box>
                        ...
                        <AiAssistButton
                            ...
                            label={t('create_campaign.campaign_goals.generate_button')}
                            variant="contained"
                            fullWidth
